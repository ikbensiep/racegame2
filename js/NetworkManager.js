    ...
    _init () {
      ...
      this.peer.on('open', (id) => {
        ...
        if (joinId) {
          // we joined an invite
          this.connect(joinId);
          this.game.inviteLink = `${window.location.origin}${window.location.pathname}?join=${joinId}&track=${this.game.scene}`;
        } else {
          this.isHost = true;
          console.info("👑 Je bent de Host. Jij beheert de bots.");
          this.game.hud.postMessage('racecontrol','notice','Welcome, King. Ur the host. 🫅', 5000)
          
          // we can invite other players
          this.game.inviteLink = `${window.location.origin}${window.location.pathname}?join=${id}&track=${this.game.scene}`;
        }
      });
